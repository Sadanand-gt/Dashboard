-- SINGLE-MONTH variant of trend_monthly_jlg.sql — driven by pipeline loop.
-- Placeholders {m_start} {m_end} {m_key} {m_label} {m_offset} are substituted
-- per month so each query stays small enough to survive the RDS hot standby.
-- =============================================================================
...
months AS (
    SELECT
        DATE '{m_start}'   AS m_start,
        DATE '{m_end}'     AS m_end,
        '{m_key}'          AS m_key,
        '{m_label}'        AS m_label,
        {m_offset}         AS m_offset
),
