-- Auto-derived from aum_status.sql (shares all DPD / demographic CTEs).
-- =============================================================================
...
-- ─────────────────────────────────────────────────────────────────────────────
-- MEMBER NAME LOOKUPS (per cust_id)
--   IL  → customer_details_il  : first_name + middle_name + last_name
--   JLG → home_borrower_master : first_name + last_name
-- concat_ws + nullif drop any blank/NULL name parts (bring only available data).
-- ─────────────────────────────────────────────────────────────────────────────
il_names AS (
    ...
    FROM public.customer_details_il
    ORDER BY cust_id
),
...
jlg_names AS (
    ...
    FROM public.home_borrower_master
    ORDER BY cust_id
),
...
il_loans AS (
    SELECT
        ...
        coalesce(nm.member_name,         'N/A')               AS member_name
    FROM public.loan_account_il la
...
-- ─────────────────────────────────────────────────────────────────────────────
-- OD LIST — one row per OD-relevant loan (current OD or OD at prev month-end)
-- ─────────────────────────────────────────────────────────────────────────────
SELECT
    b.loan_source,
    b.business_segment,
    ...
    b.loan_id,
    coalesce(b.member_name, 'N/A')         AS member_name,
    b.dpd_bucket,
    b.od_movement_status,
    b.bucket_movement,
    b.curr_od_status,
    CASE b.raw_status WHEN 'A' THEN 'Active' WHEN 'D' THEN 'Death' WHEN 'I' THEN 'Death'
         WHEN 'W' THEN 'Write-off' ELSE b.raw_status END AS loan_status,
    ...
    -- Previous slippage: had OD at previous month-end
    CASE WHEN b.eom_dpd > 0 THEN 1 ELSE 0 END AS prev_slippage,
    -- DQ Category (per .pbit / Power Pivot calc column on AUM Loandump):
    --   IF OD_DAYS>0 :  first_demand within EOMONTH(asof,-3) → Infant
    --                   first_demand within EOMONTH(asof,-8) → Early
    --   else (older first demand, or not currently OD)       → Mature
    -- EOMONTH(d,-N) = date_trunc('month',d) - (N-1) months - 1 day.
    CASE
    ...
    END                                    AS dq_category,
    b.dpd,
    round(b.pos::numeric, 2)               AS pos,
    round(b.total_arrear::numeric, 2)      AS od_amt,
    CASE WHEN b.sanctioned_amount > 0
         THEN round((b.total_arrear / b.sanctioned_amount * 100)::numeric, 2)
         ELSE 0 END                        AS od_to_disb
FROM bucketed b
LEFT JOIN hierarchy h  ON b.branch_id = h.branch_id
WHERE b.dpd > 0 OR b.eom_dpd > 0
ORDER BY b.dpd DESC, b.pos DESC;
